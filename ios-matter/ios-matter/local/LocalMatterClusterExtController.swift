...
/// attribute.
@objc public final class LocalMatterClusterExtController : NSObject {
    ...
    @objc public func generateRandomNumber(deviceId: NSNumber, endpoint: NSNumber) async throws -> Int32 {
        SwiftLogger.debug("Generating random number...")
        ...
        SwiftLogger.debug("Generating random number command succeeded.")
        
        return try await getRandomNumber(deviceId: deviceId, endpointId: endpoint)
    }
    ...
    private func getRandomNumber(deviceId: NSNumber, endpointId: NSNumber) async throws -> Int32 {
